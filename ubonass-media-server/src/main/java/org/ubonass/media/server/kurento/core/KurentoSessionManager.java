...
public class KurentoSessionManager extends SessionManager {
    ...
    public void call(Participant participant, MediaOptions mediaOptions, Integer transactionId) {
       ...
       this.publishVideo(
                kSession.getParticipantByPrivateId(participant.getParticipantPrivateId()),mediaOptions,transactionId);
        //sessionEventsHandler.onCallResponse(participant,sdpAnswer,transactionId);
        //发送result
    }

    @Override
    public void onCallAccept(Participant participant, MediaOptions mediaOptions, Integer transactionId) {
        String sessionId = participant.getSessionId();
        KurentoSession kSession = (KurentoSession) sessions.get(sessionId);
        if (kSession == null) {
            throw new OpenViduException(Code.ROOM_NOT_FOUND_ERROR_CODE,
                    "Session '" + sessionId + "' not exists");
        }
        kSession.join(participant);//会创建kurentoParticipant
        this.publishVideo(kSession.getParticipantByPrivateId(participant.getParticipantPrivateId()),mediaOptions,transactionId);
